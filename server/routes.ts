...
import { 
  ...
  bodyMeasurements, progressPhotos, nutritionGoals,
  insertUserSchema, insertWorkoutProgramSchema, insertWorkoutDaySchema,
  insertExerciseSchema, insertWorkoutDayExerciseSchema, insertWorkoutSessionSchema,
  insertWorkoutSetLogSchema, insertFoodItemSchema, insertMealSchema,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/profile/setup', requireAuth, async (req, res) => {
    try {
      const userId = req.session.userId;
      const schema = insertUserSchema.omit({ username: true, password: true, email: true });
      ...
      res.status(500).json({ message: 'An error occurred during profile setup' });
    }
  });
  ...
  app.put('/api/profile', requireAuth, async (req, res) => {
    try {
      const userId = req.session.userId;
      const schema = insertUserSchema.omit({ username: true, password: true, email: true }).partial();
  ...
  app.get('/api/exercises/:id', async (req, res) => {
    try {
      const db = getDb();
      const { id } = req.params;
      const exercise = await db.select().from(exercises).where(eq(exercises.id, parseInt(id))).limit(1);
  ...
  app.post('/api/exercises', requireAuth, async (req, res) => {
    try {
      ...
      const newExercise = await db.insert(exercises).values(result.data).returning();
  ...
  app.get('/api/workout-programs', requireAuth, async (req, res) => {
    try {
      const db = getDb();
      const userId = req.session.userId;
      const programs = await db.select().from(workoutPrograms).where(eq(workoutPrograms.userId, userId));
      res.json(programs);
    } catch (error) {
  ...
  app.post('/api/workout-programs', requireAuth, async (req, res) => {
    try {
      ...
      const newProgram = await db.insert(workoutPrograms).values(result.data).returning();
  ...
  app.get('/api/workout-programs/:programId/days', requireAuth, async (req, res) => {
    try {
      const db = getDb();
      ...
      const program = await db.select().from(workoutPrograms)
        .where(and(
          eq(workoutPrograms.id, parseInt(programId)),
      ...
      const days = await db.select().from(workoutDays)
  ...
  app.post('/api/workout-programs/:programId/days', requireAuth, async (req, res) => {
    try {
      const db = getDb();
      const { programId } = req.params;
      const userId = req.session.userId;
      ...
      const result = schema.safeParse({...req.body, programId: parseInt(programId)});
      
      if (!result.success) {
        return res.status(400).json({ message: 'Invalid input', errors: result.error.format() });
      }

      const newDay = await db.insert(workoutDays).values(result.data).returning();
  ...
  app.get('/api/workout-days/:dayId/exercises', requireAuth, async (req, res) => {
    try {
      const db = getDb();
      ...
      // Get workout day
      const day = await db.select().from(workoutDays)
        .where(eq(workoutDays.id, parseInt(dayId))).limit(1);
  ...
  app.post('/api/workout-days/:dayId/exercises', requireAuth, async (req, res) => {
    try {
      const db = getDb();
      const { dayId } = req.params;
      const userId = req.session.userId;
      ...
      const result = schema.safeParse({...req.body, dayId: parseInt(dayId)});
      
      if (!result.success) {
        return res.status(400).json({ message: 'Invalid input', errors: result.error.format() });
      }

      const newDayExercise = await db.insert(workoutDayExercises).values(result.data).returning();
      
      // Get exercise details
      const exercise = await db.select().from(exercises)
        .where(eq(exercises.id, newDayExercise[0].exerciseId)).limit(1);
  ...
  app.post('/api/workout-sessions', requireAuth, async (req, res) => {
    try {
      ...
      const newSession = await db.insert(workoutSessions).values(result.data).returning();
  ...
  app.get('/api/workout-sessions/recent', requireAuth, async (req, res) => {
    try {
      ...
      .leftJoin(workoutPrograms, eq(workoutSessions.programId, workoutPrograms.id))
  ...
  app.post('/api/workout-sessions/:sessionId/sets', requireAuth, async (req, res) => {
    try {
      const db = getDb();
      ...
      const result = schema.safeParse({...req.body, sessionId: parseInt(sessionId)});
      
      if (!result.success) {
        return res.status(400).json({ message: 'Invalid input', errors: result.error.format() });
      }

      const newSetLog = await db.insert(workoutSetLogs).values(result.data).returning();
  ...
  app.get('/api/food-items', async (req, res) => {
    try {
      ...
      if (query && typeof query === 'string') {
        results = await db.select().from(foodItems)
          .where(
  ...
  app.post('/api/meals/:mealId/items', requireAuth, async (req, res) => {
    try {
      ...
      const schema = insertMealFoodItemSchema;
      const result = schema.safeParse({...req.body, mealId: parseInt(mealId)});
      ...
      const foodItem = await db.select().from(foodItems)
        .where(eq(foodItems.id, newMealItem[0].foodItemId)).limit(1);
  ...
  app.get('/api/nutrition-goals', requireAuth, async (req, res) => {
    try {
      ...
      if (!goals.length) {
        return res.status(404).json({ message: 'No active nutrition goals found' });
      }
  ...
  app.post('/api/nutrition-goals', requireAuth, async (req, res) => {
    try {
      ...
      // Create new goal
      const newGoal = await db.insert(nutritionGoals).values(result.data).returning();
  ...
  app.get('/api/body-measurements/recent', requireAuth, async (req, res) => {
    try {
      ...
      const recentMeasurements = await db.select().from(bodyMeasurements)
  ...
  app.post('/api/body-measurements', requireAuth, async (req, res) => {
    try {
      ...
      const newMeasurement = await db.insert(bodyMeasurements).values(result.data).returning();
  ...
  app.get('/api/progress-photos', requireAuth, async (req, res) => {
    try {
      ...
      const photos = await db.select().from(progressPhotos)
  ...
  app.post('/api/progress-photos', requireAuth, async (req, res) => {
    try {
      ...
      const newPhoto = await db.insert(progressPhotos).values(result.data).returning();
